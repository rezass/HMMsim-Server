...
int main(int argc, char * argv[]) {
    ...
    OptionalArgument<double> maxFreeDram(&args, "max_free_dram", "maximum fraction of free DRAM pages (PCM migrations stop when there are more than these free pages)", 0.9);
    OptionalArgument<uint32> completeThreshold(&args, "complete_threshold", "number of blocks left to transfer that will trigger the completion of an on-demand migration", 64); //migrate 3/4 of a page
    OptionalArgument<uint64> rollbackTimeout(&args, "rollback_timeout", "number of cycles since the start of migration that triggers its rollback", 1000000);

    //Arguments for Old Hybrid Memory Manager
    OptionalArgument<MigrationMechanism> migrationMechanism(&args, "migration_mechanism", "migration mechanism (pause|pin|redirect)", PAUSE);
    ...
    OptionalArgument<bool> rollback(&args, "rollback", "whether to enable rollback of migrations", false);
    ...
    OptionalArgument<string> pcmFractions(&args, "pcm_fractions", "string representing the fraction of pcm space allocated to each process", "0.0078125"); //32MB for a 4GB system
    ...
    OptionalArgument<unsigned> completionThreshold(&args, "completion_threshold", "number of blocks left to migrate when the completion of the migration should be started", 64);
    ...
    if (memoryOrganization.getValue() == "dram") {
        ...
        dramMemory = new Memory("dram", "DRAM", &engine, &stats, debugStart.getValue(), DRAM_QUEUE, DRAM_OPEN, DRAM_ACCESS, DRAM_CLOSE, DRAM_BUS_QUEUE, DRAM_BUS, dramRowBufferPolicy.getValue(), DESTRUCTIVE_READS, dramMappingType.getValue(), dramGlobalQueue.getValue(), dramQueueSize.getValue(), dramRanks.getValue(), dramBanksPerRank.getValue(), dramRowsPerBank.getValue(), dramBlocksPerRow.getValue(), blockSize.getValue(), dramOpenLatency.getValue(), dramCloseLatency.getValue(), dramAccessLatency.getValue(), false, dramBusLatency.getValue(), 0, pcmFractions.getValue());
        pcmMemory = new Memory("pcm", "PCM", &engine, &stats, debugStart.getValue(), PCM_QUEUE, PCM_OPEN, PCM_ACCESS, PCM_CLOSE, PCM_BUS_QUEUE, PCM_BUS, pcmRowBufferPolicy.getValue(), NON_DESTRUCTIVE_READS, pcmMappingType.getValue(), pcmGlobalQueue.getValue(), pcmQueueSize.getValue(), pcmRanks.getValue(), pcmBanksPerRank.getValue(), pcmRowsPerBank.getValue(), pcmBlocksPerRow.getValue(), blockSize.getValue(), pcmOpenLatency.getValue(), pcmCloseLatency.getValue(), pcmAccessLatency.getValue(), pcmLongLatency.getValue(), pcmBusLatency.getValue(), dramMemory->getSize(), pcmFractions.getValue());
